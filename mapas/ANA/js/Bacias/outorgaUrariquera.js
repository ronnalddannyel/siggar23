...
var groupLayUrariquera = L.markerClusterGroup();

var groupLayUrariquera1 = L.layerGroup([]);


function outUrariquera(el){

  groupLayUrariquera1.clearLayers();

  var test;

  $.getJSON(UrlFemarh+"/siggarr/mapas/zee/hidrografia_ottobacias.geojson", function(data) {  
  
  var Urariquera = L.geoJson(data, {
    filter: function (features) {      
        if(features.properties.regiao48 == 'RIO URARIQUERA'){
            test = features;
            return true;
        }
    }
    ...
    if(quei12.style.display === "block"){
      ...
      if(map.hasLayer(groupLayUrariquera) || map.hasLayer(groupLayUrariquera1)){
        map.removeLayer(groupLayUrariquera);
        map.removeLayer(groupLayUrariquera1);
        document.getElementById('outUrariquera').checked = false;
        }
        else {
        map.addLayer(groupLayUrariquera);
        }
  ...
  function outUrariquera1(){


    if(map.hasLayer(groupLayUrariquera)){
      map.removeLayer(groupLayUrariquera);
      map.addLayer(groupLayUrariquera1);
    }
    else {
      map.removeLayer(groupLayUrariquera1);
      map.addLayer(groupLayUrariquera);
