...
var groupLayMapuera = L.markerClusterGroup();

var groupLayMapuera1 = L.layerGroup([]);


function outMapuera(el){

  groupLayMapuera1.clearLayers();

  var test;

  $.getJSON(UrlFemarh+"/siggarr/mapas/zee/hidrografia_ottobacias.geojson", function(data) {  
  
  var Mapuera = L.geoJson(data, {
    filter: function (features) {      
        if(features.properties.regiao48 == 'RIO MAPUERA'){
            test = features;
            return true;
        }
    }
    ...
    if(quei12.style.display === "block"){
      ...
      if(map.hasLayer(groupLayMapuera) || map.hasLayer(groupLayMapuera1)){
        map.removeLayer(groupLayMapuera);
        map.removeLayer(groupLayMapuera1);
        document.getElementById('outMapuera').checked = false;
        }
        else {
        map.addLayer(groupLayMapuera);
        }
  ...
  function outMapuera1(){


    if(map.hasLayer(groupLayMapuera)){
      map.removeLayer(groupLayMapuera);
      map.addLayer(groupLayMapuera1);
    }
    else {
      map.removeLayer(groupLayMapuera1);
      map.addLayer(groupLayMapuera);
