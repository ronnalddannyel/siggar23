...
var groupLayCatrimani = L.markerClusterGroup();

var groupLayCatrimani1 = L.layerGroup([]);


function outCatrimani(el){

  groupLayCatrimani1.clearLayers();

  var test;

  $.getJSON(UrlFemarh+"/siggarr/mapas/zee/hidrografia_ottobacias.geojson", function(data) {  
  
  var Catrimani = L.geoJson(data, {
    filter: function (features) {      
        if(features.properties.regiao48 == 'RIO CATRIMANI'){
            test = features;
            return true;
        }
    }
    ...
    if(quei12.style.display === "block"){
      ...
      if(map.hasLayer(groupLayCatrimani) || map.hasLayer(groupLayCatrimani1)){
        map.removeLayer(groupLayCatrimani);
        map.removeLayer(groupLayCatrimani1);
        document.getElementById('outCatrimani').checked = false;
        }
        else {
        map.addLayer(groupLayCatrimani);
        }
  ...
  function outCatrimani1(){


    if(map.hasLayer(groupLayCatrimani)){
      map.removeLayer(groupLayCatrimani);
      map.addLayer(groupLayCatrimani1);
    }
    else {
      map.removeLayer(groupLayCatrimani1);
      map.addLayer(groupLayCatrimani);
