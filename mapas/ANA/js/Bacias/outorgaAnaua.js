...
var groupLayAnaua = L.markerClusterGroup();

var groupLayAnaua1 = L.layerGroup([]);


function outAnaua(el){

  groupLayAnaua1.clearLayers();

  var test;

  $.getJSON(UrlFemarh+"/siggarr/mapas/zee/hidrografia_ottobacias.geojson", function(data) {  
  
  var anaua = L.geoJson(data, {
    filter: function (features) {      
        if(features.properties.regiao48 == 'RIO ANAUÃ'){
            test = features;
            return true;
        }
    }
    ...
    if(quei12.style.display === "block"){
      ...
      if(map.hasLayer(groupLayAnaua) || map.hasLayer(groupLayAnaua1)){
        map.removeLayer(groupLayAnaua);
        map.removeLayer(groupLayAnaua1);
        document.getElementById('outAnua').checked = false;
        }
        else {
        map.addLayer(groupLayAnaua);
        }
  ...
  function outAnaua1(){
    ...
    if(map.hasLayer(groupLayAnaua)){
      map.removeLayer(groupLayAnaua);
      map.addLayer(groupLayAnaua1);
    }
    else {
      map.removeLayer(groupLayAnaua1);
      map.addLayer(groupLayAnaua);
