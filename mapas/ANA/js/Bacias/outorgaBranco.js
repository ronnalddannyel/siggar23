...
var groupLayBranco = L.markerClusterGroup();

var groupLayBranco1 = L.layerGroup([]);


function outBranco(el){

  groupLayBranco1.clearLayers();

  var test;

  $.getJSON(UrlFemarh+"/siggarr/mapas/zee/hidrografia_ottobacias.geojson", function(data) {  
  
  var Branco = L.geoJson(data, {
    filter: function (features) {      
        if(features.properties.regiao48 == 'RIO BRANCO'){
            test = features;
            return true;
        }
    }
    ...
    if(quei12.style.display === "block"){
      ...
      if(map.hasLayer(groupLayBranco) || map.hasLayer(groupLayBranco1)){
        map.removeLayer(groupLayBranco);
        map.removeLayer(groupLayBranco1);
        document.getElementById('outBranco').checked = false;
        }
        else {
        map.addLayer(groupLayBranco);
        }
  ...
  function outBranco1(){
    ...
    if(map.hasLayer(groupLayBranco)){
      map.removeLayer(groupLayBranco);
      map.addLayer(groupLayBranco1);
    }
    else {
      map.removeLayer(groupLayBranco1);
      map.addLayer(groupLayBranco);
