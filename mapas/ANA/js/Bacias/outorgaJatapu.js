...
function outJatapu(el){
  ...
  var Jatapu = L.geoJson(data, {
        ...
        for(i=0; i < pol6.length; i++){
           ...
           var outSuper = L.geoJSON(pol, {
            pointToLayer: function (geom, latlng) {
                  return L.marker(latlng, {icon: Icon_e60000});
            },
    ...
    if(quei12.style.display === "block"){
      var quei1Html = '<img src="./imagens/outorga/pin-de-localizacao-e60000.png" style="max-width:20px;max-height:20px;"> Outorgas - Bacia Rio Jatapu ('+cont+')';
